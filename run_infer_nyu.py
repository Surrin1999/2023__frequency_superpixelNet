...
parser.add_argument('--data_dir', metavar='DIR', default='nyu_test/',
                    help='path to images folder')
parser.add_argument('--pretrained', metavar='PTH', help='path to pre-trained model',
                    default='./pretrain_ckpt/E905_checkpoint.tar')
...
def test(args, model, img_paths, save_path, spixeIds, idx, scale):
    ...
    ori_sz_spixel_map = F.interpolate(curr_spixl_map.type(torch.float), size=(int(448), int(608)), mode='nearest').type(
        torch.int)
    ...
    spixel_viz, spixel_label_map = get_spixel_image((ori_img + mean_values).clamp(0, 1), ori_sz_spixel_map.squeeze(),
                                                    n_spixels=1200 * scale * scale, b_enforce_connect=True)
...
def main():
    ...
    for scale in [0.5]:
        ...
        save_path = args.output + '/SPixelNet_nSpixel_{0}'.format(
            int(input_img_height / 16 * scale * input_img_width / 16 * scale))
