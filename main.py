...
parser.add_argument('--data', metavar='DIR', default='BSDS500', help='path to input dataset')
parser.add_argument('--savepath', default='checkpoint', help='path to save ckpt')
...
parser.add_argument('--epochs', default=3000, type=int, metavar='N',
                    help='number of total epoches, make it big enough to follow the iteration maxmium, 400 for BraTS and 600 for ISIC, 800 for ACDC')
...
parser.add_argument('--finetune_stone', default=0, type=int,
                    help='the epoch to use the boundary-perceiving loss to finetune')
...
parser.add_argument('--milestones', default=[54000, 216000], metavar='N', nargs='*',
...
parser.add_argument('--debug', action='store_true', default=False,
                    help='if true, read a small part of data to debug, default: false')
...
def main():
    ...
    save_path = os.path.abspath(
        args.savepath) + '/' + f'{args.dataset}{args.suffix}'  # os.path.join(args.dataset, save_path  +  '_' + timestamp )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_avg_slic, train_avg_sem, iteration = train(train_loader, model, optimizer, epoch,
                                                         spixelID, XY_feat_stack)

        # evaluate on validation set and save the module( and choose the best)
        with torch.no_grad():
            avg_slic, avg_sem = validate(val_loader, model, epoch, val_spixelID, val_XY_feat_stack)
...
def train(train_loader, model, optimizer, epoch, init_spixl_map_idx, xy_feat):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        slic_loss, loss_sem, loss_pos = compute_semantic_pos_loss(output, LABXY_feat_tensor, pix_emb, patch_posi,
                                                                  patch_label, alpha=args.alpha,
                                                                  pos_weight=args.pos_weight, kernel_size=args.downsize,
                                                                  curr_epoch=epoch, epoch_stone=args.finetune_stone)
        # ========= back propagate ===============
        optimizer.zero_grad()
        slic_loss.backward()
        optimizer.step()
...
def validate(val_loader, model, epoch, init_spixl_map_idx, xy_feat):
    global n_iter, args, intrinsic
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    with torch.no_grad():
        for i, batch_data in enumerate(val_loader):
            ...
            slic_loss, loss_sem, loss_pos = compute_semantic_pos_loss(output, LABXY_feat_tensor, pix_emb, patch_posi,
                                                                      ...
                                                                      kernel_size=args.downsize, curr_epoch=epoch,
                                                                      epoch_stone=args.finetune_stone)
            ...
            if i % args.print_freq == 0:
                print(
                    'val Epoch: [{0}][{1}/{2}]\t Time {3}\t Data {4}\t Total_loss {5}\t Loss_sem {6}\t Loss_pos {7}\t'
                        .format(epoch, i, epoch_size, batch_time, data_time, total_loss, losses_sem, losses_pos))
...
def save_checkpoint(state, is_best, filename='checkpoint.tar'):
    ...
    if is_best:
        shutil.copyfile(os.path.join(save_path, filename), os.path.join(save_path, 'pretrain_ckpt/model_best.tar'))
